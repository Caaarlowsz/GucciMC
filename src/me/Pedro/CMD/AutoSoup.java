	...
	public boolean onCommand(final CommandSender sender, final Command cmd, final String Label, final String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("autosoup")) {
			if (!p.hasPermission("slicemc.admin")) {
				p.sendMessage(String.valueOf(String.valueOf(GucciPvP.p)) + " �7Sem premiss\u00e3o !");
				return true;
			}
			if (args.length == 0) {
			...
			if (target == null || !(target instanceof Player)) {
				sender.sendMessage(String.valueOf(String.valueOf(GucciPvP.p)) + " �7Este jogador se encontra offline !");
				return true;
			}
			if (target.getName() == p.getName()) {
				p.sendMessage(
						String.valueOf(GucciPvP.p) + " �7Voc\u00ea n\u00e3o pode usar o autosoup em voc\u00ea mesmo !");
				return true;
			}
