...
public class Fake implements Listener, CommandExecutor
{
        ...
        if (sender instanceof Player) {
            ...
            if (cmd.getName().equalsIgnoreCase("fake")) {
                if (p.hasPermission("kitpvp.fake")) {
                    if (args.length == 0) {
                        p.sendMessage(String.valueOf(String.valueOf("§b§lFANNT§c§lKITS §c")) + "§cSintaxe correta: /fake (nome)");
                        return false;
                    }
                    if (args.length == 1) {
                        if (args[0].equalsIgnoreCase("#")) {
                            ...
                            if (Fake.usandoFake.contains(p)) {
                                p.sendMessage(String.valueOf(String.valueOf("§b§lFANNT§c§lKITS")) + "§cEscreva: /fake # §2\u2714");
                                return true;
                            }
                            if (args[0].length() > 16) {
                                p.sendMessage(String.valueOf(String.valueOf("§b§lFANNT§c§lKITS")) + "§cnick grande demais escolha outro §4\u2717");
                                return true;
                            }
    ...
    public void refreshPlayer(final Player player, final Player forWhom) {
        ...
        if (player != forWhom && player.getWorld() == forWhom.getWorld() && forWhom.canSee(player)) {
            ...
            Bukkit.getServer().getScheduler().scheduleSyncDelayedTask((Plugin)Main.instance, (Runnable)new Fake.Fake$1(this, forWhom, player), 2L);
        }
    }
