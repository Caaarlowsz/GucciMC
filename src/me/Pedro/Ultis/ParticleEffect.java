...
public enum ParticleEffect
{
    ...
    static {
        ...
        try {
            ParticleEffect.packetPlayOutWorldParticles = (Constructor<?>)ReflectionHandler.getConstructor(ReflectionHandler.PacketType.PLAY_OUT_WORLD_PARTICLES.getPacket(), new Class[] { String.class, Float.TYPE, Float.TYPE, Float.TYPE, Float.TYPE, Float.TYPE, Float.TYPE, Float.TYPE, Integer.TYPE });
            ...
            ParticleEffect.sendPacket = ReflectionHandler.getMethod((Class)ParticleEffect.playerConnection.getType(), "sendPacket", new Class[] { ReflectionHandler.getClass("Packet", ReflectionHandler.PackageType.MINECRAFT_SERVER) });
        }
        catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static ParticleEffect fromName(final String name) {
        if (name != null) {
            for (final Map.Entry e : ParticleEffect.NAME_MAP.entrySet()) {
    ...
    private static Object instantiatePacket(final String name, final Location center, final float offsetX, final float offsetY, final float offsetZ, final float speed, final int amount) {
        if (amount < 1) {
            throw new ParticleEffect.PacketInstantiationException("Amount cannot be lower than 1");
        }
        try {
            ...
            throw new ParticleEffect.PacketInstantiationException("Packet instantiation failed", (Throwable)e);
        }
    }
    ...
    private static void sendPacket(final Player p, final Object packet) {
        try {
            ...
            throw new ParticleEffect.PacketSendingException("Failed to send a packet to player '" + p.getName() + "'", (Throwable)e);
        }
    }
    ...
    public void display(final Location center, final float offsetX, final float offsetY, final float offsetZ, final float speed, final int amount, final Player players) {
        sendPacket(Arrays.asList(players), instantiatePacket(this.name, center, offsetX, offsetY, offsetZ, speed, amount));
    }
    
    public void display(final Location center, final double range, final float offsetX, final float offsetY, final float offsetZ, final float speed, final int amount) {
        if (range > 16.0) {
