    ...
    protected void generateFrames() {
        for (int index = 0; index < this.context.length(); ++index) {
            if (this.context.charAt(index) != ' ') {
                final String highlighted = String.valueOf(String.valueOf(this.normalFormat)) + this.context.substring(0, index) + this.highlightFormat + this.context.charAt(index) + this.normalFormat + this.context.substring(index + 1, this.context.length());
                final String whole = String.valueOf(String.valueOf(this.prefix)) + highlighted + this.suffix;
                this.addFrame(whole);
            }
            else {
                this.addFrame(String.valueOf(String.valueOf(this.prefix)) + this.normalFormat + this.context + this.suffix);
            }
        }
    }
