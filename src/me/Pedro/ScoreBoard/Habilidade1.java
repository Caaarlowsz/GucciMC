...
public class Habilidade1 implements Listener {
	public static Map<String, String> powerMap;

	static {
		Habilidade1.powerMap = new HashMap<String, String>();
	}

	public Habilidade1(final GucciPvP main) {
	}
	...
	public static String getAbility(final Player player) {
		if (!Habilidade1.powerMap.containsKey(player.getName())) {
			Habilidade1.powerMap.put(player.getName(), "None");
		}
		return Habilidade1.powerMap.get(player.getName());
	}

	public static void setAbility(final Player player, final String ability) {
		Habilidade1.powerMap.put(player.getName(), ability);
	}

	public static void removeAbility(final Player p) {
		Habilidade1.powerMap.remove(p.getName());
	}
