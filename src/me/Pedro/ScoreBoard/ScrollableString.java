...
public class ScrollableString extends FrameAnimatedString
{
    ...
    public ScrollableString(String message, int width, int spaceBetween) {
        ...
        for (int j = 0; j < spaceBetween; ++j) {
            this.list.add(String.valueOf(String.valueOf(message.substring(message.length() - width + ((j > width) ? width : j), message.length()))) + (Object)space);
            if (space.length() < width) {
                space.append(" ");
            }
        }
        for (int j = 0; j < width - spaceBetween; ++j) {
            this.list.add(String.valueOf(String.valueOf(message.substring(message.length() - width + spaceBetween + j, message.length()))) + (Object)space + message.substring(0, j));
        }
        for (int j = 0; j < spaceBetween && j <= space.length(); ++j) {
            this.list.add(String.valueOf(String.valueOf(space.substring(0, space.length() - j))) + message.substring(0, width - ((spaceBetween > width) ? width : spaceBetween) + j));
        }
    }
