...
{
    private static final String Methodos;
    ...
    public void onDeath(final PlayerDeathEvent e) throws SQLException {
        ...
        if (p.getKiller() instanceof Player) {
            ...
            p.sendMessage(String.valueOf(String.valueOf(Main.p)) + " §7Voc\u00ea foi §cmorto §7por: §b" + k.getName() + " §7!");
            KitAPI.remove(p);
            KitAPI.Kit.remove(p);
        }
    }
    ...
    public void abrirClassses(final PlayerInteractEvent evento) {
        ...
        if (jogador.getItemInHand().getType().equals((Object)Material.AIR) && jogador.getItemInHand().getItemMeta().hasDisplayName() && jogador.getItemInHand().getItemMeta().getDisplayName().equalsIgnoreCase("")) {
            evento.setCancelled(true);
            evento.getAction();
            final Action right_CLICK_AIR = Action.RIGHT_CLICK_AIR;
        }
    }
    ...
    public void abrirClasssedddds(final PlayerInteractEvent evento) {
        final Player jogador = evento.getPlayer();
        if (jogador.getItemInHand().getType().equals((Object)Material.CHEST) && jogador.getItemInHand().getItemMeta().hasDisplayName()) {
