...
public class DropEventos implements Listener
{
    ...
    public void onItemDrop(final ItemSpawnEvent e) {
        Bukkit.getServer().getScheduler().scheduleSyncDelayedTask((Plugin)Main.getInstance(), (Runnable)new DropEventos.DropEventos$1(this, e), 20L);
    }
    ...
    public void aoDropar(final PlayerDropItemEvent e) {
        ...
        if (e.getItemDrop().getItemStack().getType() == Material.FURNACE) {
            e.setCancelled(true);
        }
        ...
        if (e.getItemDrop().getItemStack().getType() == Material.MINECART) {
            e.setCancelled(true);
        }
        if (e.getItemDrop().getItemStack().getType() == Material.STAINED_GLASS_PANE) {
            e.setCancelled(true);
